...
import { driversAPI, ridesAPI } from "./Api";
...
export default function PassengerDashboard() {
  ...
  useEffect(() => {
    ...
    const onCancelled = (ride) => {
      if (String(ride.passenger?._id || ride.passenger) === String(user?._id)) {
        setActiveRide(ride);
        showToast("Ride was cancelled", "warn");
      }
    };
    const onDriverStatus = () => driversAPI.online().then(d => setDrivers(Array.isArray(d) ? d : [])).catch(() => {});
  ...
  const submitRating = async () => {
    ...
    try {
      await ridesAPI.rate(activeRide._id, rating, feedback);
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1.25rem" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
              ...
              <div style={{ background:"linear-gradient(135deg,#0d0f15,#0a0c12)", border:`1px solid ${STATUS_COLOR[activeRide.status]}33`, borderRadius:16, padding:"1.5rem", animation:"fadeUp .3s ease", position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:"1.25rem" }}>
                  ...
                  <div style={{ display:"flex", justifyContent:"flex-end" }}>
                    <span style={{ fontSize:20, fontWeight:800, fontFamily:"Space Mono,monospace", color:"#00ff99" }}>₹{activeRide.fare}</span>
                  </div>
                </div>
                ...
                {activeRide.status==="requested" && (
                  <button onClick={cancelRide} style={{ marginTop:"1rem", padding:"7px 16px", border:"1px solid rgba(255,77,109,0.3)", borderRadius:8, background:"rgba(255,77,109,0.06)", color:"#ff4d6d", fontSize:12, cursor:"pointer" }}>Cancel Ride</button>
                )}
              </div>
            )}
            ...
            {activeRide?.status==="completed" && !activeRide.rating && (
              <div style={{ background:"linear-gradient(135deg,#0d0f15,#0a0c12)", border:"1px solid rgba(123,94,167,0.3)", borderRadius:16, padding:"1.5rem", animation:"fadeUp .3s ease" }}>
                ...
                <input value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Optional feedback..." style={{ width:"100%", padding:"10px 14px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, color:"#e8eaf0", fontFamily:"DM Sans,sans-serif", fontSize:13, outline:"none", marginBottom:12, boxSizing:"border-box" }} />
            ...
            <div style={{ background:"linear-gradient(135deg,#0d0f15,#0a0c12)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:16, padding:"1.5rem" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, marginBottom:6 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, marginBottom:"1.25rem" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
            <div style={{ background:"linear-gradient(135deg,#0d0f15,#0a0c12)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:16, padding:"1.5rem" }}>
              ...
              {drivers.length===0 && <div style={{ textAlign:"center", padding:"1.5rem", color:"#4a5168", fontSize:13 }}><div style={{ fontSize:28, marginBottom:8 }}>🛺</div>No drivers online right now</div>}
              {drivers.slice(0,4).map(d => (
                <div key={d._id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 12px", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:10, marginBottom:8 }}>
            ...
            <div style={{ background:"linear-gradient(135deg,#0d0f15,#0a0c12)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:16, padding:"1.5rem", flex:1 }}>
              ...
              {history.slice(0,4).map(r => (
                <div key={r._id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:10, marginBottom:8 }}>
